...
async function main() {
  ...
  // Contract addresses (replace with actual deployed addresses if different)
  ...
  // Mock backend signer (for testing; replace with actual private key in production)
  const backendSignerPrivateKey = process.env.BACKENDSIGNERPRIVATEKEY; 
  const backendSigner = new ethers.Wallet(backendSignerPrivateKey, ethers.provider);
  ...
  // Setup Basenames
  console.log("Setting up Basenames...");
  await resolver.connect(owner).setResolvedAddress(ethers.namehash("creator.base.eth"), owner.address, { gasLimit: 300000 });
  await resolver.connect(owner).setResolvedAddress(ethers.keccak256(ethers.toUtf8Bytes("player1.base.eth")), player1.address, { gasLimit: 300000 });
  await resolver.connect(owner).setResolvedAddress(ethers.keccak256(ethers.toUtf8Bytes("player2.base.eth")), player2.address, { gasLimit: 300000 });
  console.log("Basenames set:");
  console.log("creator.base.eth ->", owner.address);
  console.log("player1.base.eth ->", player1.address);
  console.log("player2.base.eth ->", player2.address);
  console.log("Verifying Basenames:");
  console.log("creator:", await resolver.resolve(ethers.namehash("creator.base.eth")));
  console.log("player1:", await resolver.resolve(ethers.keccak256(ethers.toUtf8Bytes("player1.base.eth"))));
  console.log("player2:", await resolver.resolve(ethers.keccak256(ethers.toUtf8Bytes("player2.base.eth"))));
  ...
  // Generate signatures for players
  const generateSignature = async (playerAddress, basename, gameId) => {
    const messageHash = ethers.keccak256(
      ethers.defaultAbiCoder.encode(["address", "string", "uint256"], [playerAddress, basename, gameId])
    );
    const ethSignedHash = ethers.keccak256(
      ethers.toUtf8Bytes("\x19Ethereum Signed Message:\n32" + messageHash.slice(2))
    );
    return await backendSigner.signMessage(ethers.utils.arrayify(ethSignedHash));
  };
  const player1Signature = await generateSignature(player1.address, "player1.base.eth", gameId);
  const player2Signature = await generateSignature(player2.address, "player2.base.eth", gameId);

  // Players join game
  console.log("Players joining game...");
  await game.connect(player1).joinGame(gameId, "player1.base.eth", player1Signature, { gasLimit: 500000 });
  await game.connect(player2).joinGame(gameId, "player2.base.eth", player2Signature, { gasLimit: 500000 });
  ...
  // Parse Transfer events to get minted NFT tokenIds
  console.log("Fetching minted NFT tokenIds...");
  const transferEvents = receiptEnd.logs
  ...
  console.log("Player1 balance:", ethers.formatEther(await token.balanceOf(player1.address)), "tokens (Expected: 12.8)");
  console.log("Player2 balance:", ethers.formatEther(await token.balanceOf(player2.address)), "tokens (Expected: 3)");
  console.log("Staking contract:", ethers.formatEther(await token.balanceOf(stakingAddress)));
}
