     ...
     interface IBasenameResolver {
         ...
         function getBasename(address addr) external view returns (string memory);
         function getTwitterUsername(address addr) external view returns (string memory);
     }
     ...
     contract DTAIOCGame is Ownable, ReentrancyGuard {
         ...
         function createGame(
             ...
             if (hasBasename) {
                 ...
                 string memory resolvedBasename = basenameResolver.getBasename(msg.sender);
                 if (keccak256(abi.encodePacked(resolvedBasename)) != keccak256(abi.encodePacked(basename))) {
                     revert UnauthorizedCaller();
                 }
                 creator = msg.sender;
             } else {
                 string memory resolvedTwitter = basenameResolver.getTwitterUsername(msg.sender);
                 if (keccak256(abi.encodePacked(resolvedTwitter)) != keccak256(abi.encodePacked(twitterUsername))) {
                     revert UnauthorizedCaller();
                 }
                 creator = msg.sender;
         ...
         function endGame(uint256 gameId) public onlyOwner nonReentrant {
             ...
             if (game.ended) revert GameAlreadyEnded();
            //  if (block.timestamp <= game.startTime + game.gameDuration) revert GameDurationExceeded();
             if (block.timestamp < game.startTime) revert GameDurationExceeded();
         ...
         function isPerfectScore(uint256 gameId, address player, uint256 stage) external view returns (bool) {
             Player storage playerData = gamesData[gameId].players[player];
             return playerData.score >= 5 && stage == 3;
         }
